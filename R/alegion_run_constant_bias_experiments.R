...
damage_tib %>% write_csv("fashion-mnist-const-damage-results.csv")

damage_tib <- read_csv("fashion-mnist-const-damage-results.csv") %>%
  mutate(unbiased = 1-frac)

alegion_pal <- c("#e47e25", "#1e386a", "#289cb9", "#4e5464")
...
ggsave(filename=here::here("plot/black-acc-const-bias.png"),
       width = 16 * (1/3),
       height = 9 * (1/3),
       dpi = 300)
...
ggsave(filename=here::here("plot/black-auc-const-bias.png"),
       width = 16 * (1/3),
       height = 9 * (1/3),
       dpi = 300)
